...
class marshal_data_channel_state_tests : public data_channel_tests,
...
TEST_P(marshal_data_channel_state_tests, marshal_as_native_DataState_to_managed_RtcDataChannelState_test)
{
	...
	const auto result = marshal_as<RtcDataChannelState>(from);

	const auto expected_lval = expected; //The Microsoft compiler apparently has some trouble with structured binding of managed value types (enums). This forces a copy
...
TEST_P(marshal_data_channel_state_tests, marshal_as_managed_RtcDataChannelState_to_native_DataState_test)
{
	...
	const auto result = marshal_as<DataChannelInterface::DataState>(unwrapped_from);
...
TEST(marshal_data_channel_state_tests, marshal_as_native_dataState_to_managed_rtcdatachannelstate_invalid_test)
{
	try
	{
		const auto _ = marshal_as<RtcDataChannelState>(
